...
Public Class ConditionForm
    ...
    Dim IdCondition As Integer
    Dim UrlTarifasRest As String = ConfigurationManager.AppSettings("urlServiciosTarifa")

    Private Sub ConditionForm_Load(sender As Object, e As EventArgs) Handles Me.Load
    ...
    Private Sub bbiExport_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiExport.ItemClick
        ExportToExcel(gcMainDataCondition)
    End Sub
    ...
    Private Sub bbiRefresh_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiRefresh.ItemClick
        BarButtonEnabled()
        LoadMainData()
        gcMainDataCondition.Enabled = True
        SplitContainerControl1.PanelVisibility = SplitPanelVisibility.Panel2
    End Sub
    ...
    Private Sub LoadMainData()
        ...
        If response.IsSuccessStatusCode Then
            ...
            Dim listCondiciones As List(Of ConditionClient) = Newtonsoft.Json.JsonConvert.DeserializeObject(Of List(Of ConditionClient))(jsonString)
            gcMainDataCondition.DataSource = listCondiciones
            Refresh()
    ...
    Private Sub GridView1_FocusedRowChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs) Handles GridView1.FocusedRowChanged
        ...
        Dim dgrItem As ConditionClient = GridView1.GetRow(GridView1.FocusedRowHandle)
    ...
    Private Sub LoadInputValidations()
        ...
        'vpInputs.SetValidationRule(Me.lueCondicion, customValidationRule)
    End Sub
    ...
    Private Sub bbiInsert_ItemClick(sender As Object, e As ItemClickEventArgs) Handles bbiInsert.ItemClick
        ...
        gcMainDataCondition.Enabled = False
    ...
    Private Sub LoadInputs()
        ...
        If dgrItem IsNot Nothing Then
            IdCondition = dgrItem.IdCondition
            'txtCodigo.EditValue = dgrItem.ConditionCode 'dgrItem("GroupCode")
            txtDescripcion.EditValue = dgrItem.ConditionDescription 'dgrItem("GroupDescription")
            'oConditionClient.IdUnit = 5
            txtMaxValor.Text = dgrItem.MaxValue.ToString()
            txtMinValor.Text = dgrItem.MinValue.ToString()
            lueEstado.EditValue = dgrItem.ConditionStatus
    ...
    Private Sub bbiDelete_ItemClick(sender As Object, e As ItemClickEventArgs) Handles bbiDelete.ItemClick
        If DevExpress.XtraEditors.XtraMessageBox.Show("La eliminación del registro seleccionado podría afectar la integridad de la información, desea continuar?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = System.Windows.Forms.DialogResult.Yes Then
            ...
            'Dim response2 As HttpResponseMessage = client.DeleteAsync(UrlTarifasRest + "/Condition/DeleteCondicion" + "/" + dgrItem.IdCondition.ToString()).Result
            Dim response2 As HttpResponseMessage = client.PutAsync(UrlTarifasRest + "/Condition/UpdateCondicion", content).Result
    ...
    Private Sub bbiSave_ItemClick(sender As Object, e As ItemClickEventArgs) Handles bbiSave.ItemClick
        ...
        Dim oConditionClient As ConditionClient = New ConditionClient()
        oConditionClient.ConditionCode = "" 'txtCodigo.Text
        oConditionClient.ConditionDescription = txtDescripcion.Text
        oConditionClient.MaxValue = Convert.ToDecimal(txtMaxValor.Text)
        oConditionClient.MinValue = Convert.ToDecimal(txtMinValor.Text)
        oConditionClient.ConditionStatus = lueEstado.EditValue.ToString().Trim()
        oConditionClient.IdUnit = lueUnidad.EditValue
        'oConditionClient.UserCreate = 1
        'oConditionClient.UserUpdate = 1

        oConditionClient.IdBusinessUnit = lueUnidadNegocio.EditValue
        ...
        If DevExpress.XtraEditors.XtraMessageBox.Show("Está seguro de guardar los cambios?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = System.Windows.Forms.DialogResult.Yes Then
            Try
                If Index = 0 Then
                    oConditionClient.UserCreate = AppUser
                    oConditionClient.DateCreate = DateTime.Now
                    Dim jsonSerialice As String = Newtonsoft.Json.JsonConvert.SerializeObject(oConditionClient)
                    Dim content As StringContent = New StringContent(jsonSerialice, Encoding.UTF8, "application/json")
                    Dim response2 As HttpResponseMessage = client.PostAsync(UrlTarifasRest + "/Condition/InsertarCondicion", content).Result
                Else
                    oConditionClient.UserUpdate = AppUser
                    oConditionClient.IdCondition = IdCondition
                    oConditionClient.DateUpdate = DateTime.Now
                    Dim jsonSerialice As String = Newtonsoft.Json.JsonConvert.SerializeObject(oConditionClient)
                    Dim content As StringContent = New StringContent(jsonSerialice, Encoding.UTF8, "application/json")
                    Dim response2 As HttpResponseMessage = client.PutAsync(UrlTarifasRest + "/Condition/UpdateCondicion", content).Result
                End If
