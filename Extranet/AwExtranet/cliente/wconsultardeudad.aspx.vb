...
Public Class wconsultardeudad
    ...
    Public Shared Function fnsearchHblByCustomer(ByVal scode As Integer, ByVal shbl As String) As enAjax
        ...
        Try
            ...
            dt = oServicio.CustomerDebtQuery(scode, shbl)

            Dim valor As String = ""
            If dt.Rows.Count > 0 Then
                For Each row As DataRow In dt.Rows
                    valor += CStr(row("HBL")) + "," + CStr(row("Grupo")) + "," + CStr(row("TipoDocumento")) + "," + CStr(row("Concepto")) + "," + CStr(row("Moneda")) + "," + CStr(row("Importe")) + "," + CStr(row("FormaPago")) + "," + CStr(row("DOCV_FechaVcmto")) + "|"
                Next
