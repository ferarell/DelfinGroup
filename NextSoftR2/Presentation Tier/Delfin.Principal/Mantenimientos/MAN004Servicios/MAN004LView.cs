...
namespace Delfin.Principal
{
    public partial class MAN004LView : UserControl, IMAN004LView
    {
        ...
        {
            if (Convert.ToInt32(grdItems.CurrentRow.Cells["SERV_CodUnificador"].Value) > 0)
            {
                try
                { throw new System.ArgumentException("Sólo se pueden sincronizar los servicios que no estén unificados.");}
                catch (Exception ex)
                {
                    Infrastructure.WinForms.Controls.Dialogos.MostrarMensajeError(Presenter.Title, "No es posible sincronizar un servicio unificado.", ex);
                    return;
                }             
