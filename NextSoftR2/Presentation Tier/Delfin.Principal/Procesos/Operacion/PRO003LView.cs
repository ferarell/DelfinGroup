...
namespace Delfin.Principal
{
   public partial class PRO003LView : UserControl, IPRO003LView
   {
      ...
      //    }
        ...
        private void insertarEventoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            {
                ApplicationForm.EventManagementForm oEventManagementForm;
                oEventManagementForm = new ApplicationForm.EventManagementForm();
                oEventManagementForm.AppUser = Presenter.Session.UserName;
                oEventManagementForm.teHBL.Text = grdItems.CurrentRow.Cells["COPE_HBL"].Value.ToString();
                oEventManagementForm.bInsert = true;
                oEventManagementForm.Show();
                //oMailCargoNoticeForm.lueNoticeLayout.Enabled = false;
                //oMailCargoNoticeForm.bbiSearch.PerformClick();
            }
        }

        private void btnGenerarAviso_Click(object sender, EventArgs e)
        {
            ApplicationForm.MailCargoNoticeForm oMailCargoNoticeForm;
            oMailCargoNoticeForm = new ApplicationForm.MailCargoNoticeForm();
            oMailCargoNoticeForm.AppUser = Presenter.Session.UserName;
            oMailCargoNoticeForm.teHBL.Text = grdItems.CurrentRow.Cells["COPE_HBL"].Value.ToString();
            oMailCargoNoticeForm.bSearch = true;
            oMailCargoNoticeForm.Show();
            oMailCargoNoticeForm.lueNoticeLayout.EditValue = "003";
        }
    }
}
