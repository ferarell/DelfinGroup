...
Public Class DetraccionesPagosForm
        ...
        If vpInputs.Validate Then
            Try
                If rbProcessType.SelectedIndex = 0 Then
                    If ExisteLote() Then
                        msg = IIf(gbFileGenerate.SelectedIndex = 0, "El lote indicado ya existe, desea reemplazarlo?", "El lote indicado ya existe, desea continuar?")
                        If DevExpress.XtraEditors.XtraMessageBox.Show(msg, "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.No Then
                            Return
                        End If
                    End If
                    TransactionData()
    ...
    Friend Function ExisteLote() As Boolean
        bExisteLote = False
        Dim sQuery As String = "select * from PagosDetracciones where [Numero RUC Empresa] = '" & lueSociedad.GetColumnValue("RUC") & "' and [Numero Lote] = '" & seLote.Text & "'"
    ...
    Private Sub TransactionData()
        ...
        Try
            ...
            End If
            If gbFileGenerate.SelectedIndex = 0 Then
                If bProcess Then
                    If EliminaLote() Then
                        If ActualizaLote() Then
    ...
    Friend Function EliminaLote() As Boolean
        ...
        bResult = ExecuteAccessNonQuery("DELETE FROM PagosDetracciones where [Numero RUC Empresa] = '" & lueSociedad.GetColumnValue("RUC") & "' and [Numero Lote] = '" & seLote.Text & "'")
        Return bResult
    End Function
    ...
    Friend Function ActualizaLote() As Boolean
        ...
        For Each row As DataRow In dtDetail.Rows
            ...
            dtPagos.Rows(iPos).Item(1) = lueSociedad.GetColumnValue("RUC")
        ...
        Try
            ...
            For Each row As DataRow In dtSource.Rows
                If Not IsDBNull(row(0)) Then
                    ImpDoc = GetAmountByDoc(IIf(IsDBNull(row(4)), 0, row(4)), IIf(IsDBNull(row(5)), 0, row(5)), IIf(IsDBNull(row(7)), 0, row(7)), 10)
                    ...
                    If Not IsDBNull(row(1)) Then
                        dtDetail.Rows(iPosition).Item("C2") = Strings.Left(row(1).ToString.Trim & Space(11), 11)
                    End If
                    If Not IsDBNull(row(0)) Then
                        dtDetail.Rows(iPosition).Item("C3") = Strings.Left(row(0).ToString.Trim & Space(35), 35)
                    End If
                    dtDetail.Rows(iPosition).Item("C4") = "000000000"
                    dtDetail.Rows(iPosition).Item("C5") = "037"
                    dtDetail.Rows(iPosition).Item("C6") = Strings.Left(GetAccVendor(row(1).ToString.Trim) & Space(11), 11)
            ...
            'Cabecera
            dtHeader.Rows.Add()
            dtHeader.Rows(0).Item("C1") = "*"
            dtHeader.Rows(0).Item("C2") = lueSociedad.GetColumnValue("RUC")
    ...
    Private Sub SearchData()
        ...
        If teRUC.Text.Trim <> "" Then
            sCondition &= IIf(sCondition = "", " where ", " and ") & "[Numero RUC Empresa]='" & lueSociedad.GetColumnValue("RUC") & "'"
        End If
        If tePeriodo.Text.Trim <> "" Then
            sCondition &= IIf(sCondition = "", " where ", "") & "[Periodo]='" & tePeriodo.Text & "'"
        End If
        If teRUC.Text.Trim <> "" Then
    ...
    Private Sub rbProcessType_SelectedIndexChanged(sender As Object, e As EventArgs) Handles rbProcessType.SelectedIndexChanged
        LoadInputValidations()
        FiltersEnabled()
        seLote.EditValue = 0
    End Sub
    ...
    Private Sub PoblarSociedad()
        lueSociedad.Properties.DataSource = FillDataTable("Sociedad$")
    ...
    Private Sub lueSociedad_EditValueChanged(sender As Object, e As EventArgs) Handles lueSociedad.EditValueChanged
        If rbProcessType.SelectedIndex = 0 Then
            seLote.EditValue = ExecuteAccessQuery("select iif(IsNull(max([Numero Lote])),  0,  max([Numero Lote])) + 1 as Lote from PagosDetracciones where [Numero RUC Empresa] = '" & lueSociedad.GetColumnValue("RUC") & "'").Tables(0).Rows(0)(0)
    ...
    Private Sub GridView1_RowCellStyle(ByVal sender As Object, ByVal e As RowCellStyleEventArgs) Handles GridView1.RowCellStyle
        If rbProcessType.SelectedIndex = 1 Then
            Return
        End If
