...
Public Class ValidacionesPLEForm
    Dim RUC, SunatFileName1, SunatFileName2 As String
    Dim LibroSunat As String = "LibroDiario"
    Dim dsLibroSunat As New dsSunat
    Dim dsExcel As New DataSet
    Dim dtTypePaytDoc, dtAccountMapping, dtResult1, dtResult2, dtSales, dtPurchases As New DataTable
    Dim bFlatFileGenerate As Boolean = True
    Dim bProcess As Boolean = True

    Private Sub ValidacionesPLEForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        seEjercicio.Value = Today.Year
        sePeriodo.Value = Today.Month
        DBFileName = My.Settings.DBDirectory & "\" & My.Settings.DBFileName
        PoblarSociedad()
    ...
    Private Sub PoblarSociedad()
        Dim dtSociedad As New Data.DataTable
        dtSociedad = FillDataTable("Sociedad$")
        lueSociedad.Properties.DataSource = dtSociedad
        lueSociedad.Properties.DisplayMember = "Descripción"
        lueSociedad.Properties.ValueMember = "Código"
    End Sub
    ...
    Private Sub LoadInputValidations()
        ...
        vpInput.SetValidationRule(Me.seEjercicio, customValidationRule)
        vpInput.SetValidationRule(Me.seEjercicio, customValidationRule)
        vpInput.SetValidationRule(Me.beArchivoOrigen, customValidationRule)
        vpInput.SetValidationRule(Me.beArchivoSalida1, customValidationRule)
    End Sub
    ...
    Private Sub LoadAccountMapping()
        dtAccountMapping = FillDataTable("MapeoCtas" & lueSociedad.EditValue & "$")
    End Sub
    ...
    Private Sub bbiCerrar_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs)
        Close()
    End Sub

    Private Sub bbiProcesar_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs)
        Me.Refresh()
        LoadAccountMapping()
        bFlatFileGenerate = True
        bProcess = True
        If vpInput.Validate Then
            Try
    ...
    Private Sub ProcessLedger()
        ...
        Try
            ...
            For Each row As DataRow In dtSource.Rows
                ...
                End If
            Next
            dtAccounts = SelectDistinct(dtSource, "CtaCtb", "CtaDes")
    ...
    Friend Sub NewRowLedger1(row As DataRow)
        ...
        For icol = 0 To row.ItemArray.Length - 1
        ...
        Try
            ...
            dtResult1.Rows(iPosition).Item("C4") = row(4)
            ...
            dtResult1.Rows(iPosition).Item("C15") = Format(row(1), "dd/MM/yyyy")
            ...
            If row(4).trim = "" Then
                ...
                dtResult1.Rows(iPosition).Item("C22") = "No existe equivalencia para la cuenta: " & row(11)
            End If
        Catch ex As Exception
    ...
    Friend Sub NewRowLedger2(row As DataRow)
        ...
        Try
            ...
            dtResult2.Rows(iPosition).Item("C3") = row(1)
            dtResult2.Rows(iPosition).Item("C4") = "01"
            dtResult2.Rows(iPosition).Item("C5") = ""
    ...
    Private Sub lueSociedad_EditValueChanged(sender As Object, e As EventArgs) Handles lueSociedad.EditValueChanged, seEjercicio.EditValueChanged, sePeriodo.EditValueChanged, lueCompany.EditValueChanged
        If lueSociedad.EditValue <> "" Then
            RUC = lueSociedad.GetColumnValue("RUC")
    ...
    Private Sub beArchivoVentas_Properties_ButtonClick(sender As Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs)
    ...
    Private Sub beArchivoCompras_Properties_ButtonClick(sender As Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs)
        OpenFileDialog1.Filter = "Excel Files (*.xls*)|*.xls*|Text files (*.txt)|*.txt"
        OpenFileDialog1.FileName = ""
    ...
    Friend Function GetLocalAccount(chart As String, type As String) As String
        ...
        If dtAccountMapping.Select("[G/L Account] = '" & chart & "'").Length > 0 Then
            If type = "0" Then
                If Not IsDBNull(dtAccountMapping.Select("[G/L Account] = '" & chart & "'")(0).Item(2)) Then
                    sResult = dtAccountMapping.Select("[G/L Account] = '" & chart & "'")(0).Item(2)
                End If
            Else
                If Not IsDBNull(dtAccountMapping.Select("[G/L Account] = '" & chart & "'")(0).Item(3)) Then
                    sResult = dtAccountMapping.Select("[G/L Account] = '" & chart & "'")(0).Item(3)
                End If
            End If
        End If
        Return sResult
