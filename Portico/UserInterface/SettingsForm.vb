...
Imports MailBot.InterfazUsuario.AppService

Public Class SettingsForm
    Dim dtInputType, dtStructure, dtInputList, dtIdentifier, dtConfiguration As New DataTable
    Dim oProxyService As New AppService.PorticoClient
    ...
    Private Sub LoadData()
        gcInputType.DataSource = Nothing
        gcInputList.DataSource = Nothing
        dtInputType = ExecuteSQL("EXEC ptc.upGetInputTypeById NULL")
        If dtInputType.Rows.Count > 0 Then
            gcInputType.DataSource = dtInputType
        End If
        dtStructure = ExecuteSQL("EXEC ptc.upGetMessageStructure")
    ...
    Private Sub bbiSave_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiSave.ItemClick
        For r = 1 To dtStructure.Rows.Count
            If r = GridView2.GetFocusedRowCellValue("IdMessageStructure") Then
                ...
                If dtConfiguration.Select("IdMessageStructure=" & GridView2.GetFocusedRowCellValue("IdMessageStructure")).Length = 0 Then
    ...
    Private Sub GetConfiguration(GridName As String)
        ...
        gcStructure.DataSource = Nothing
        If GridName = "GridView3" Then
            'gcInputList.DataSource = Nothing
            If Not GridView3.GetFocusedRowCellValue("IdInputType") Is Nothing Then
                dtInputList.Rows.Clear()
                dtInputList = ExecuteSQL("EXEC ptc.upGetInputConfigurationByType " & GridView3.GetFocusedRowCellValue("IdInputType"))
        ...
        If GridName = "GridView1" Then
            ...
            If Not GridView1.GetFocusedRowCellValue("IdInputConfiguration") Is Nothing Then
                ...
                dtIdentifier = ExecuteSQL("EXEC ptc.upGetIdentifiersById " & GridView1.GetFocusedRowCellValue("IdInputConfiguration"))
        ...
        If dtInputList.Rows.Count = 0 Or GridView1.GetFocusedRowCellValue("IdInputConfiguration") Is Nothing Then
            Return
        End If
        Dim _MsgStructure As String = "NULL"
        If Not GridView2.GetFocusedRowCellValue("IdMessageStructure") Is Nothing Then
            _MsgStructure = GridView2.GetFocusedRowCellValue("IdMessageStructure")
        End If
        dtConfiguration = ExecuteSQL("EXEC ptc.upGetConfigurationById " & GridView1.GetFocusedRowCellValue("IdInputConfiguration") & ", " & _MsgStructure)
        ...
        If GridView2.RowCount > 0 Then
            If dtConfiguration.Select("IdMessageStructure=" & GridView2.GetFocusedRowCellValue("IdMessageStructure")).Length > 0 Then
                Dim dtMsgTxt As New DataTable
                dtMsgTxt = dtConfiguration.Select("IdMessageStructure=" & GridView2.GetFocusedRowCellValue("IdMessageStructure").ToString).CopyToDataTable
                If dtMsgTxt.Rows.Count > 0 Then
                    recText.HtmlText = UnicodeBytesToString(dtMsgTxt.Rows(0)("MessageText"))
                End If
            End If
        End If
    End Sub
